...
alias update='paru -Syyu && sudo flatpak update'                                    # Update all packages (Pacman, AUR, Flatpak)
alias mirror='sudo rate-mirrors --allow-root --save /etc/pacman.d/mirrorlist arch && sudo pacman -Syy'  # Update mirror list
alias clean='sudo pacman -Sc --noconfirm && paru -Sc --noconfirm && sudo flatpak uninstall --unused && sudo pacman -Rns --noconfirm $(pacman -Qtdq) 2>/dev/null'  # Clean package cache and orphans
